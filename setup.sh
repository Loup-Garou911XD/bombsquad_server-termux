...
update_termux(){
    ...
    apt-get upgrade -o Dpkg::Options::="--force-confnew" --allow -y &>>$log_file
}


update_ubuntu(){
    proot-distro login ubuntu &>>$log_file -- apt-get update && apt-get upgrade -o Dpkg::Options::="--force-confnew" --allow -y  &>>$log_file
}
